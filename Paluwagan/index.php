...
if (isset($_POST['register'])) {
    ...
    if (mysqli_num_rows($check) > 0) {
        ...
        // FIXED: Explicitly insert 'pending' into the status column so the account goes straight to the admin dashboard queue
        $insertUser = "INSERT INTO users (username, first_name, last_name, email, phone, occupation, address, password_hash, role, status)
                    VALUES ('$username', '$first_name', '$last_name', '$email', '$phone', '$occupation', '$address', '$password', 'member', 'pending')";
